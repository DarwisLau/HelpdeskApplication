        ...
        if formDetails[1] == 1:
            ...
            SELECT a.administratorEmail
             FROM Administrator a, AdministratorViewingOfForm v
              WHERE
               v.formIsSolved = 1 AND
               v.formNumber = ? AND
               v.administratorID = a.administratorID;
            ''',
            (dataFormNumber,))
            administratorSolved = dbCursor.execute()
...
def viewImage(dataClientRole, dataClientEmail, dataFormNumber):
    ...
    try:
        ...
        #Check whether there is an image attached to the Helpdesk Form
        dbCursor.execute('''
        SELECT IIF(
         (SELECT formImage FROM Form
          WHERE formNumber = ?) = ?,
        ...
        hasImage = dbCursor.fetchone()
        if hasImage[0] == 0:
            return "There is no image attached to the Helpdesk Form."
...
def setFormAsSolved (dataAdministratorEmail, dataFormNumber):
    ...
    try:
        ...
        VALUES(
         ...
         ?);
        ''',
        (dataAdministratorEmail, dataAFormNumber,))
        dbConnection.commit()
